...
/*const wchar_t* procName{L"ac_client.exe"};
	// Get procId of the target process  getProcId(  )
	...
	uintptr_t pDynamicBAseAddress{ modBaseAddr + 0x11E20C };

	std::cout << "DynamicBAaseAddress: 0x" << std::hex << pDynamicBAseAddress << '\n';
	while ( true )
	{

	// Resolve our ammo pointer chain
	std::vector<unsigned int> ammoOffsets={0x374, 0x14, 0x0};
	uintptr_t ammoAddr{ findDMAAddy( hProcess, pDynamicBAseAddress, ammoOffsets ) };

	std::cout << "AmmoAddress: 0x" << std::hex << ammoAddr << '\n';

	// Read ammo value
	int ammoVal{ 0 };
	ReadProcessMemory( hProcess, (BYTE*)ammoAddr, &ammoVal, sizeof( ammoVal ), nullptr );

	std::cout << "Current AmmoVal: " << std::dec << ammoVal << '\n';
	...
	std::cout << "New AmmoVal: " << std::dec << ammoVal << '\n';
	if ( getchar( ) == 'y' )
		break;
	}
	*/
